...
# Model classes
from MLC.GUI.Experiment.Model.ConfigurationTabModel import ConfigurationTabModel
from MLC.GUI.Experiment.Model.ExperimentsTabModel import ExperimentsTabModel
from MLC.GUI.Experiment.Model.FirstIndividualsTabModel import FirstIndividualsTabModel
from MLC.GUI.Experiment.Model.ResultsTabModel import ResultsTabModel

# View Classes
from MLC.GUI.Experiment.View.ConfigurationTabView import ConfigurationTabView
from MLC.GUI.Experiment.View.ExperimentsTabView import ExperimentsTabView
from MLC.GUI.Experiment.View.FirstIndividualsTabView import FirstIndividualsTabView
from MLC.GUI.Experiment.View.ResultsTabView import ResultsTabView
...
class ExperimentWindow(QMainWindow):
    ...
    def __init__(self, mlc_local,
        ...
        # View classes
        self._conf_view = ConfigurationTabView(experiment_name=experiment_name,
                                               parent=self,
                                               autogen_object=self._autogenerated_object)
        self._experiments_view = ExperimentsTabView(experiment_name=experiment_name,
                                                    parent=self,
                                                    autogen_object=self._autogenerated_object)
        self._first_indivs_view = FirstIndividualsTabView(experiment_name=experiment_name,
                                                          parent=self,
                                                          autogen_object=self._autogenerated_object)
        self._results_view = ResultsTabView(experiment_name=experiment_name,
                                            parent=self,
                                            autogen_object=self._autogenerated_object)

        # Model classes
        self._conf_model = ConfigurationTabModel(mlc_local=self._mlc_local,
                                                 experiment_name=experiment_name)
        self._experiments_model = ExperimentsTabModel(mlc_local=self._mlc_local,
                                                      experiment_name=experiment_name)
        self._first_indivs_model = FirstIndividualsTabModel(mlc_local=self._mlc_local,
                                                            experiment_name=experiment_name)
        self._results_view = ResultsTabModel(mlc_local=self._mlc_local,
                                             experiment_name=experiment_name)

        # Create the bindings
        self._first_indivs_model.register_observer_add_individual(self._first_indivs_view.individual_added)
        self._first_indivs_model.register_observer_modify_individual(self._first_indivs_view.individual_modified)
        self._first_indivs_model.register_observer_remove_individual(self._first_indivs_view.individual_removed)

    def closeEvent(self, event):
        logger.debug('[EXPERIMENT {0}] [CLOSE_DIALOG] - '
                     'Executing overriden closeEvent function'
                     .format(self._experiment_name))

    def on_start_button_clicked(self):
        logger.debug('[EXPERIMENT {0}] [START_BUTTON] - '
                     'Executing on_start_button_clicked function'
                     .format(self._experiment_name))

    def on_prev_gen_button_clicked(self):
        logger.debug('[EXPERIMENT {0}] [PREV_GEN_BUTTON] - '
                     'Executing on_prev_gen_button_clicked function'
                     .format(self._experiment_name))

    def on_next_gen_button_clicked(self):
        logger.debug('[EXPERIMENT {0}] [NEXT_GEN_BUTTON] - '
                     'Executing on_next_gen_button_clicked function'
                     .format(self._experiment_name))

    def on_test_button_clicked(self):
        logger.debug('[EXPERIMENT {0}] [TEST_BUTTON] - '
                     'Executing on_test_button_clicked function'
                     .format(self._experiment_name))

    def on_log_check_clicked(self):
        logger.debug('[EXPERIMENT {0}] [LOG_CHECK_CLICKED] - '
                     'Executing on_log_check_clicked function'
                     .format(self._experiment_name))

    def on_show_all_check_clicked(self):
        logger.debug('[EXPERIMENT {0}] [SHOW_ALL_CHECK_CLICKED] - '
                     'Executing on_show_all_check_clicked function'
                     .format(self._experiment_name))

    def on_dimension_check_clicked(self):
        logger.debug('[EXPERIMENT {0}] [DIMENSION_CHECK_CLICKED] - '
                     'Executing on_dimension_check_clicked function'
                     .format(self._experiment_name))
        # TODO: Don't know what the 3D graphic option should do yet...

    def on_save_config_button_clicked(self):
        logger.debug('[EXPERIMENT {0}] [SAVE_CONFIG_BUTTON_CLICKED] - '
                     'Executing on_save_config_button_clicked function'
                     .format(self._experiment_name))

    def on_tab_changed(self, tab_index):
        logger.debug('[EXPERIMENT {0}] [TAB_CHANGED] - '
                     'Executing on_tab_changed function'
                     .format(self._experiment_name))

    def on_import_config_button_clicked(self):
        logger.debug('[EXPERIMENT {0}] [IMPORT_CONFIG_BUTTON_CLICKED] - '
                     'Executing on_import_config_button_clicked function'
                     .format(self._experiment_name))

    def on_export_config_button_clicked(self):
        logger.debug('[EXPERIMENT {0}] [EXPORT_CONFIG_BUTTON_CLICKED] - '
                     'Executing on_export_config_button_clicked function'
                     .format(self._experiment_name))

    def on_ev_edit_button_clicked(self):
        logger.debug('[EXPERIMENT {0}] [EV_EDIT_BUTTON_CLICKED] - '
                     'Executing on_ev_edit_button_clicked function'
                     .format(self._experiment_name))

    def on_preev_edit_button_clicked(self):
        logger.debug('[EXPERIMENT {0}] [PREEV_EDIT_BUTTON_CLICKED] - '
                     'Executing on_preev_edit_button_clicked function'
                     .format(self._experiment_name))

    def on_gen_count_combo_changed(self, generation):
        logger.debug('[EXPERIMENT {0}] [GEN_COUNT_COMBO_CHANGED] - '
                     'Executing on_gen_count_combo_changed function'
                     .format(self._experiment_name))

    def on_chart_config_changed(self, value_changed):
        logger.debug('[EXPERIMENT {0}] [CHART_CONFIG_CHANGED] - '
                     'Executing on_chart_config_changed function'
                     .format(self._experiment_name))

    def on_best_indiv_button_clicked(self):
        logger.debug('[EXPERIMENT {0}] [BEST_INDIV_BUTTON] - '
                     'Executing on_best_indiv_button_clicked function'
                     .format(self._experiment_name))

    def on_convergence_button_clicked(self):
        logger.debug('[EXPERIMENT {0}] [CONVERGENCE_BUTTON] - '
                     'Executing on_convergence_button_clicked function'
                     .format(self._experiment_name))

    def on_genealogy_button_clicked(self):
        logger.debug('[EXPERIMENT {0}] [GENEALOGY_BUTTON] - '
                     'Executing on_genealogy_button_clicked function'
                     .format(self._experiment_name))
    ...
    def on_first_add_indiv_button_clicked(self):
        ...
        if indiv[1] == True:
            ...
            if not indiv[0]:
                ...
                return
        self._first_indivs_model.add_individual(indiv[0])
...
### OLD EXPERIMENT WINDOWS ###
# Create the bindings
    #     self._first_indivs_model.register_observer_add_invididual(self._first_indivs_view.individual_added)
    #     self._first_indivs_model.register_observer_modify_invididual(self._first_indivs_view.individual_modified)
    #     self._first_indivs_model.register_observer_remove_invididual(self._first_indivs_view.individual_removed)
