...
class ExperimentInProgressDialog(QDialog):
    ...
    def _update_current_gen_experiment(self, indiv_index, cost):
        # FIXME: Hardcoded values. Set them as constants and see how the user could change them
        if cost > 900:
            self._indiv_chart.append_point(1, indiv_index, -50)
            return

        if math.isnan(cost) or math.isinf(cost):
            self._indiv_chart.append_point(2, indiv_index, -50)
            return
    ...
    def _create_new_chart(self):
        ...
        # Blue: Valid points
        indiv_chart.add_scatter(marker_size=5, color=Qt.green, legend="Valid Points")
        # Red: Overflow points
        indiv_chart.add_scatter(marker_size=5, color=Qt.red, legend="Overflow")
        # Yellow: Nan and Inf points
        indiv_chart.add_scatter(marker_size=5, color=Qt.black, legend="Nan or Inf")

        indiv_canvas = indiv_chart.get_widget()
        indiv_canvas.chart().axisX().setRange(1, self._indivs_per_gen)
        indiv_canvas.chart().axisY().setRange(-100, 900)
        self._max_cost = 0
