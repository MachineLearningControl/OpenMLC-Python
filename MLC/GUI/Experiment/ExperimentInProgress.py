...
class ExperimentInProgressDialog(QDialog):
    ...
    def __init__(self, parent, parent_signal):
    ...
    def _update_current_gen_experiment(self, indiv_index, cost):
        # self._autogenerated_object.picture_layout.itemAt(0).widget().repaint()
        # chart_view = self._autogenerated_object.picture_layout.itemAt(0).widget()
        # chart = chart_view.chart()
        # indiv_chart = QtChartWrapper(chart_view=chart_view, chart=chart)
        if cost == float('NaN') or cost == float('Inf'):
            cost = 0

        if cost > self._max_cost:
            self._max_cost = cost
            self._indiv_chart.get_widget().chart().axisY().setRange(-10, self._max_cost)
    ...
    def _create_new_chart(self):
        ...
        indiv_chart.set_title('Generation {0} - Cost Per Individual'.format(self._current_generation))
