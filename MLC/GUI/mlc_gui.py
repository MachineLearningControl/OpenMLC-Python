...
class MLC_GUI(QMainWindow):
    ...
    def on_export_button_clicked(self):
        ...
        try:
            export_path = os.path.join(export_dir, self._experiment_selected + ".tar.gz")
            source_path = os.path.join(self._mlc_local.get_working_dir(), self._experiment_selected)
            self._make_tarfile(export_path, source_path)
            logger.info("[MLC_MANAGER] [EXPORT_BUTTON] - An error ocurred while exporting project {0}"
                        .format(self._experiment_selected))
            QMessageBox.information(self, "Ëxperiment Exported",
                                    "Experiment {0} was succesfully exported. It is stored in {1}"
                                    .format(self._experiment_selected, export_path))
        except Exception, err:
            logger.error("[MLC_MANAGER] [EXPORT_BUTTON] - An error ocurred while exporting project {0}."
                         "Error {1}".format(self._experiment_selected, err))
            QMessageBox.critical(self, "Experiment Not Exported", "Experiment could not be exported")
