...
class SerialConnection(BaseConnection):
    ...
    def __init__(self, **args):
        ...
        args["baudrate"] = 115200 if "baudrate" not in args.keys() else args[
            "baudrate"]
        args["parity"] = serial.PARITY_NONE if "parity" not in args.keys() else args[
            "parity"]
        args["stopbits"] = serial.STOPBITS_ONE if "stopbits" not in args.keys() else args[
            "stopbits"]
        args["bytesize"] = serial.EIGHTBITS if "bytesize" not in args.keys() else args[
            "bytesize"]
        args["timeout"] = 5
