...
RUN git clone git://code.qt.io/qt/qtbase.git && \
    cd qtbase && \
    git checkout v5.9.1 && \
    ./configure --prefix=/opt/mlc-python-3.4.7/Qt-5.9.1 -xkb-config-root /usr/share/X11/xkb -no-gtk -nomake tests -nomake examples -qt-xcb --opensource --confirm-license && \
